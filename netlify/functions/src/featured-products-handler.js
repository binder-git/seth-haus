...
console.log('[Featured Products] Initializing with environment variables:', {
    ...
    ALL_ENV_KEYS: Object.keys(process.env).filter(key => key.startsWith('COMMERCE_LAYER_') ||
        key === 'NODE_ENV' ||
...
const handler = async (event) => {
    try {
        ...
        // Get access token using centralized auth endpoint
        ...
        try {
            ...
            const client = new sdk_1.CommerceLayerClient({
                organization,
...
//# sourceMappingURL=featured-products-handler.js.map