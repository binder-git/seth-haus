...
describe('CommerceLayerContext', () => {
    const mockEuMarket = { id: 'market:id:qjANwhQrJg', name: 'EU', region: 'eu', countryCode: 'EU', currencyCode: 'EUR' };
    const mockUkMarket = { id: 'market:id:vjzmJhvEDo', name: 'UK', region: 'uk', countryCode: 'GB', currencyCode: 'GBP' };
    // Mock Commerce Layer SDK
    ...
    test('Initial Market Configuration', () => {
        const { result } = renderHook(() => useCommerceLayer(), {
            wrapper: ({ children }) => (_jsx(CommerceLayerProvider, { children: children }))
        });
